window.dom = {
    ...
    style (node, name, value) {
        if (arguments.length === 3) //dom.style(node,'color','red')
        node.style[name] = value;
        else if (arguments.length === 2) {
            if (typeof name === "string") //dom.style(node,'border')
    ...
    next (node) {
        ...
        while(x && x.nodeType === 3)return x.nextSibling;
        return x;
    },
    previous (node) {
        let x = node.previousSibling;
        while(x && x.nodeType === 3)return x.previousSibling;
        return x;
    },
    each (nodeList, fn) {
